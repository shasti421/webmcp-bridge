...
const toolStepSchema = {
  oneOf: [
      ...
      properties: {
        capture: {
          ...
          properties: {
            ...
            store_as: { type: 'string' as const },
            wait: { type: 'boolean' as const },
            on_failure: { type: 'boolean' as const },
          },
        },
      },
    },
    ...
    },
    {
      ...
      additionalProperties: false,
      properties: {
        evaluate_js: { type: 'string' as const },
      },
    },
  ],
};
...
const workflowSchema = {
  ...
  properties: {
    ...
    input: {
      type: 'object' as const,
      minProperties: 1,
      additionalProperties: workflowParamSchema,
    },
    output: {
      type: 'object' as const,
      additionalProperties: workflowParamSchema,
  ...
  private buildValidationError(ajvErrors: ErrorObject[]): BridgeError {
    ...
    for (const error of ajvErrors) {
      ...
      if (keyword === 'required') {
        ...
        // oneOf failures are often redundant noise when nested; skip
        continue;
      } else {
