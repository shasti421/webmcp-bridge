...
describe('YamlSchemaValidator', () => {
  ...
  describe('validateApp()', () => {
    ...
    it('rejects url_patterns as string instead of array', () => {
      const result = validator.validateApp({ ...validApp, url_patterns: '/path' });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error.code).toBe('SCHEMA_VALIDATION_ERROR');
    ...
    it('rejects invalid base_url format', () => {
      const result = validator.validateApp({ ...validApp, base_url: 'not-a-url' });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error.code).toBe('SCHEMA_VALIDATION_ERROR');
    ...
    it('rejects invalid id pattern (uppercase)', () => {
      const result = validator.validateApp({ ...validApp, id: 'MyApp' });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error.code).toBe('SCHEMA_VALIDATION_ERROR');
    ...
    it('rejects extra unknown fields', () => {
      const result = validator.validateApp({ ...validApp, unknown_field: 'value' });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error.code).toBe('SCHEMA_VALIDATION_ERROR');
      }
    ...
    it('rejects empty url_patterns array', () => {
      const result = validator.validateApp({ ...validApp, url_patterns: [] });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error.code).toBe('SCHEMA_VALIDATION_ERROR');
      }
  ...
  describe('validatePage()', () => {
    const validPage = {
      ...
      fields: [
        {
          id: 'new_todo_input', label: 'New Todo', type: 'text',
          selectors: [
        ...
        {
          id: 'add_button', label: 'Add Todo', type: 'action_button',
          selectors: [
      ...
      outputs: [
        {
          id: 'todo_items', label: 'Todo Items',
          selectors: [{ strategy: 'css', selector: '.todo-item .todo-text' }],
        },
      ],
    };

    it('accepts a valid page definition', () => {
      expect(validator.validatePage(validPage).ok).toBe(true);
    });

    it('rejects page with only 1 field (minimum 2)', () => {
      const result = validator.validatePage({ ...validPage, fields: [validPage.fields[0]] });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error.code).toBe('SCHEMA_VALIDATION_ERROR');
    ...
    it('rejects page with empty outputs', () => {
      const result = validator.validatePage({ ...validPage, outputs: [] });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error.code).toBe('SCHEMA_VALIDATION_ERROR');
    ...
    it('rejects field with empty selectors', () => {
      const result = validator.validatePage({
        ...validPage,
        fields: [{ ...validPage.fields[0], selectors: [] }, validPage.fields[1]],
      });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error.code).toBe('SCHEMA_VALIDATION_ERROR');
    ...
    it('rejects invalid selector strategy', () => {
      const result = validator.validatePage({
        ...validPage,
        fields: [
          { ...validPage.fields[0], selectors: [{ strategy: 'invalid', something: 'bad' }] },
          validPage.fields[1],
        ],
      });
      expect(result.ok).toBe(false);
    ...
    it('accepts page with optional overlays', () => {
      const result = validator.validatePage({
        ...validPage,
        overlays: [{
          id: 'cookie-banner', trigger: '.cookie-overlay',
    ...
    it('accepts output without capture_strategies (optional)', () => {
      const result = validator.validatePage({
        ...validPage,
        outputs: [{ id: 'status', label: 'Status', selectors: [{ strategy: 'css', selector: '.status' }] }],
      });
      expect(result.ok).toBe(true);
    });
    ...
    it('reports multiple validation errors in a single message', () => {
      const result = validator.validatePage({
        id: 'test', app: 'test-app', url_pattern: '/',
        fields: [{
  ...
  describe('validateTool()', () => {
    ...
    it('accepts a valid tool definition', () => {
      expect(validator.validateTool(validTool).ok).toBe(true);
    });
    ...
    it('rejects tool with empty steps', () => {
      const result = validator.validateTool({ ...validTool, bridge: { ...validTool.bridge, steps: [] } });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error.code).toBe('SCHEMA_VALIDATION_ERROR');
      }
    ...
    it('accepts tool with page ref that does not exist (validation only)', () => {
      expect(validator.validateTool({ ...validTool, bridge: { ...validTool.bridge, page: 'nonexistent' } }).ok).toBe(true);
    });

    it('accepts tool with capture step', () => {
      const result = validator.validateTool({
        ...validTool,
        bridge: { ...validTool.bridge, steps: [{ navigate: { page: 'todo_list' } }, { capture: { from: 'todo_count', store_as: 'count', wait: true } }] },
      });
      expect(result.ok).toBe(true);
    });

    it('accepts tool with wait step', () => {
      expect(validator.validateTool({ ...validTool, bridge: { ...validTool.bridge, steps: [{ wait: 1000 }] } }).ok).toBe(true);
    });

    it('accepts tool with evaluate_js step', () => {
      expect(validator.validateTool({ ...validTool, bridge: { ...validTool.bridge, steps: [{ evaluate_js: 'document.title' }] } }).ok).toBe(true);
    });

    it('rejects tool with invalid name pattern', () => {
      const result = validator.validateTool({ ...validTool, name: 'Add-Todo' });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error.code).toBe('SCHEMA_VALIDATION_ERROR');
      }
  ...
  describe('validateWorkflow()', () => {
    const validWorkflow = {
      ...
      input: { items: { type: 'array', required: true, description: 'Items to add' } },
      steps: [
        { for_each: '{{items}}', as: 'item', on_error: 'continue', steps: [{ tool: 'add_todo', params: { text: '{{item}}' } }] },
        { aggregate: { added_count: '{{loop.successes.length}}' } },
      ],
    };

    it('accepts a valid workflow definition', () => {
      expect(validator.validateWorkflow(validWorkflow).ok).toBe(true);
    });

    it('rejects workflow with no steps', () => {
      const result = validator.validateWorkflow({ ...validWorkflow, steps: [] });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error.code).toBe('SCHEMA_VALIDATION_ERROR');
        expect(result.error.message).toContain('1');
    ...
    it('rejects workflow missing required fields', () => {
      const result = validator.validateWorkflow({ name: 'test_workflow' });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error.code).toBe('SCHEMA_VALIDATION_ERROR');
      }
    ...
    it('accepts workflow with output definition', () => {
      expect(validator.validateWorkflow({ ...validWorkflow, output: { total: { type: 'number', description: 'Total items' } } }).ok).toBe(true);
    });

    it('rejects workflow with invalid name pattern', () => {
      const result = validator.validateWorkflow({ ...validWorkflow, name: 'Batch-Add' });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error.code).toBe('SCHEMA_VALIDATION_ERROR');
      }
    ...
    it('rejects workflow with empty input', () => {
      const result = validator.validateWorkflow({ ...validWorkflow, input: {} });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error.code).toBe('SCHEMA_VALIDATION_ERROR');
      }
